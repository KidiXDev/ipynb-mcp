...
func RegisterInsertMarkdownCell(s *server.MCPServer) {
	tool := mcp.NewTool(
		...
		mcp.WithDescription("Insert a markdown cell at a given index."),
		...
		mcp.WithString("source", mcp.Required(), mcp.Description("Markdown source for the new cell.")),
	)
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		return withNotebookMutation(path, func(nb *notebook.Notebook) error {
			return notebook.InsertMarkdownCell(nb, index, source)
		})
	})
}
